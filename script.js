...
// ---------- DEV SKIP ----------
// In the browser console, type: skipToScene(3)
// This lets you jump straight to a scene without clicking through everything.
// Remove this block (or just don't call it) in the final version.
 
const sceneStartSteps = {
  1: 0,   // step index where Scene 1 begins
  2: 11,  // step index where Scene 2 begins (after transition + scene_setup)
  3: 34,  // step index where Scene 3 begins (after transition + scene_setup)
  4: 46   // step index where Scene 4 begins (after transition + scene_setup)
};
 
window.skipToScene = function(sceneNumber) {
  const targetStep = sceneStartSteps[sceneNumber];
  if (targetStep === undefined) {
    console.warn("Unknown scene:", sceneNumber, "— valid values are 1, 2, 3, 4");
    return;
  }
 
  // Show the interior if not already visible
  document.getElementById("exterior").style.display = "none";
  document.getElementById("interior").style.display = "block";
  ...
  // Run all scene_setup functions up to the target step
  // so the right characters are visible
  for (let i = 0; i < targetStep; i++) {
    if (scene[i].type === "scene_setup") {
      scene[i].fn();
    }
  }
 
  // Jump to the target step and run it
  step = targetStep;
  next();
 
  console.log(`Skipped to Scene ${sceneNumber} (step ${targetStep})`);
};